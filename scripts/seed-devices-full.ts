...
const devices = [
  ...
  { brand: 'Realme', model: 'Realme 10', storage: '4GB RAM 64GB', launchPrice: 13999, launchDate: '2022-11-09', basePriceExcellent: 5500, basePriceGood: 4500, basePriceAverage: 3500 },
...
async function seed() {
  console.log(`\n🚀 Starting full device seed — ${devices.length} devices to upsert...\n`);
  ...
  for (const device of devices) {
    try {
      ...
      console.log(`  ✅ ${device.brand} ${device.model} (${device.storage})`);
    } catch (err: any) {
      failed++;
