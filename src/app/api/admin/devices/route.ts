...
export async function POST(req: Request) {
  try { await requireAdmin(req); const input = deviceSchema.parse(await req.json()); const device = await prisma.deviceMaster.create({ data: { ...input, isActive: input.isActive ?? true } }); return jsonResponse({ success: true, device }, 201); }
  catch (error: any) { if (error instanceof z.ZodError) return jsonResponse({ error: 'Validation failed', details: error.errors }, 400); if (error.code === 'P2002') return jsonResponse({ error: 'A device with this brand, model, and storage already exists' }, 409); return jsonResponse({ error: error.message || 'Failed to create device' }, error.message === 'Forbidden' ? 403 : 500); }
}
...
export async function PATCH(req: Request) {
  try {
    ...
    if (error instanceof z.ZodError) return jsonResponse({ error: 'Validation failed', details: error.errors }, 400);
    return jsonResponse({ error: error.message || 'Failed to update device' }, error.message === 'Forbidden' ? 403 : 500);
  }
}
